              ...
              <Link
                ...
                className={
                  action.variant === 'primary'
                    ? 'inline-flex items-center gap-2 rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition-transform duration-300 hover:-translate-y-0.5'
          ...
          >
            {homeMetrics.map((metric) => (
              ...
              >
                ...
                <div className="mt-2 font-serif text-4xl text-slate-900">{metric.value}</div>
                {metric.detail ? (
        ...
        >
          ...
          <div className="grid gap-5 md:grid-cols-[0.65fr_1fr] xl:grid-cols-2">
            ...
            <div className="rounded-[1.8rem] border border-[#e6dbc9] bg-[linear-gradient(135deg,rgba(118,43,40,0.08),rgba(202,171,119,0.18),rgba(255,255,255,0.92))] p-6">
              <div className="text-xs uppercase tracking-[0.22em] text-primary/70">Wofür die Seite steht</div>
              <h2 className="mt-3 font-serif text-3xl text-slate-900">Konservativ, aber nicht statisch.</h2>
              <p className="mt-4 leading-7 text-slate-700">
                Die neue Seite zeigt nicht nur Texte, sondern den tatsächlichen Lebensraum der Teutonia:
                Haus, Räume, Nutzung, Alltagsrhythmus und den Übergang von Studium zu Gemeinschaft.
              </p>
            </div>
          </div>
        </motion.div>
